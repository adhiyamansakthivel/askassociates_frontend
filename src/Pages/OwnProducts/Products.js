...
const Products = () => {
  ...
  const apiProductList = useAPIGet(
    "productsList",
    "productsList",
    `${location?.state?.data?.itemURL === "all" ? APIEndPoints.GetAllOwnProducts.url : APIEndPoints.GetOwnProductList_Category?.url + location?.state?.data?.itemURL
    }`,
    {
      ...
      onSuccess: (res) => {
        if (location?.state?.data?.itemURL === "all") {
          ...
          if (res.status == 200) {
            if (res?.data?.data?.[0]?.ownproducts?.length !== 0) {
              setViewList(res.data?.data?.[0]?.ownproducts);
            }
          }
          else {
            setViewList([]);
  ...
  const navLinkClickHandler = (productDetails) => {
    if (productDetails?.slug_url) {
      navigate(`/manufacturedetail/${productDetails?.slug_url}`, {
        state: {
          data: {
